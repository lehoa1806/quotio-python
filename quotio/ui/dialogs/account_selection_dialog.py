    ...
    def _populate_accounts(self):
        ...
        for row, auth_file in enumerate(self.auth_files):
            ...
            if quota_info and quota_info.models:
                ...
                percentages = [m.percentage for m in quota_info.models if m.percentage is not None and m.percentage >= 0]
                if percentages:
            ...
            if quota_info and quota_info.models:
                # Filter models with valid percentage and sort by percentage (ascending - lowest remaining first)
                valid_models = [
                    m for m in quota_info.models 
                    if m.percentage is not None and m.percentage >= 0
                ]
                if valid_models:
