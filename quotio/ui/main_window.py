...
_async_loop: Optional[asyncio.AbstractEventLoop] = None  # The asyncio event loop instance
_loop_thread: Optional[threading.Thread] = None  # Thread running the event loop
_loop_running: bool = False  # Flag indicating if loop is currently running


def setup_async_loop(exception_handler: Optional[Callable] = None) -> asyncio.AbstractEventLoop:
    """
    ...
    def run_loop():
        ...
        try:
            ...
            # Clean up any remaining tasks before marking loop as stopped
            try:
                # Get all remaining tasks
                remaining_tasks = [t for t in asyncio.all_tasks(_async_loop) if not t.done()]
                if remaining_tasks:
...
def shutdown_async_loop() -> None:
    """
    Safely shutdown the async event loop and wait for thread to finish.

    This function:
    1. Cancels pending tasks and waits for them to complete
    2. Stops the event loop gracefully
    3. Waits for the loop thread to finish
    4. Cleans up resources
    """
    global _async_loop, _loop_thread, _loop_running

    if _async_loop is None or not _loop_running:
        print("[AsyncLoop] Event loop not running, nothing to shutdown")
        return

    print("[AsyncLoop] Shutting down event loop...")

    try:
        # Cancel all pending tasks first
        pending_tasks = []
        try:
            pending_tasks = [task for task in asyncio.all_tasks(_async_loop) if not task.done()]
            if pending_tasks:
                print(f"[AsyncLoop] Cancelling {len(pending_tasks)} pending tasks...")
                for task in pending_tasks:
                    try:
                        task.cancel()
                    except Exception as e:
                        print(f"[AsyncLoop] Error cancelling task: {e}")
        except Exception as e:
            print(f"[AsyncLoop] Error getting pending tasks: {e}")

        # Give cancelled tasks a moment to complete their cleanup
        # This is important for aiohttp tasks which have nested futures that need cleanup
        if pending_tasks:
            import time
            # Give tasks time to handle cancellation and clean up
            # aiohttp tasks especially need time to close connections
            time.sleep(0.5)  # Brief pause for cleanup
            
            # Check how many tasks completed
            completed = [t for t in pending_tasks if t.done()]
            if completed:
                print(f"[AsyncLoop] {len(completed)} task(s) completed cancellation cleanup")
            
            remaining = [t for t in pending_tasks if not t.done()]
            if remaining:
                # These will be cleaned up when the loop closes
                print(f"[AsyncLoop] {len(remaining)} task(s) still pending (will be cleaned up by loop closure)")
        ...
        try:
            ...
            print(f"[AsyncLoop] Error stopping event loop: {e}")

        # Wait for loop to stop (with timeout)
        import time
        timeout = 5.0  # 5 seconds timeout
        start_time = time.time()
        while _loop_running and (time.time() - start_time) < timeout:
            time.sleep(0.1)

        if _loop_running:
            print("[AsyncLoop] Warning: Event loop did not stop within timeout")
        else:
            print("[AsyncLoop] Event loop stopped successfully")

        # Wait for thread to finish (with timeout)
        if _loop_thread and _loop_thread.is_alive():
            print(f"[AsyncLoop] Waiting for thread {_loop_thread.name} to finish...")
            _loop_thread.join(timeout=2.0)
            if _loop_thread.is_alive():
                print("[AsyncLoop] Warning: Thread did not finish within timeout (it's a daemon thread, will terminate with main thread)")
            else:
                print("[AsyncLoop] Thread finished successfully")

        # Close the loop
        # Note: Closing the loop will clean up any remaining pending tasks
        try:
            if not _async_loop.is_closed():
                _async_loop.close()
                print("[AsyncLoop] Event loop closed")
        except Exception as e:
            print(f"[AsyncLoop] Error closing event loop: {e}")

    except Exception as e:
        print(f"[AsyncLoop] Error during shutdown: {e}")
        import traceback
        traceback.print_exc()
    finally:
        _async_loop = None
        _loop_thread = None
        _loop_running = False
        print("[AsyncLoop] Shutdown complete")


def process_asyncio_tasks() -> None:
    """
...
class MainWindow:
    ...
    def _on_status_bar_context_menu(self, position: QPoint) -> None:
    ...
    def _copy_status_bar_text(self) -> None:
    ...
    def _select_all_status_bar_text(self) -> None:
    ...
    def _initialize_viewmodel(self) -> None:
    ...
    def _register_quota_callbacks(self) -> None:
    ...
    def _cleanup(self) -> None:
        ...
        if self.view_model:
            # Ensure proxy is stopped (synchronous fallback)
            # This ensures proxy stops even if async cleanup fails
            try:
                if (self.view_model.proxy_manager and
                    self.view_model.proxy_manager.proxy_status.running):
                    print("[MainWindow] Stopping proxy (synchronous fallback)...")
                    self.view_model.proxy_manager.stop()
                    print("[MainWindow] Proxy stopped (synchronous)")
            except Exception as e:
                print(f"[MainWindow] Error stopping proxy (synchronous): {e}")
            ...
            try:
                ...
                # Fallback: try to stop proxy synchronously if async failed
                try:
                    if (self.view_model.proxy_manager and
                        self.view_model.proxy_manager.proxy_status.running):
                        print("[MainWindow] Stopping proxy (fallback after async failure)...")
                        self.view_model.proxy_manager.stop()
                except Exception as e2:
                    print(f"[MainWindow] Error stopping proxy (fallback): {e2}")
        ...
        # Shutdown async loop after cleanup
        shutdown_async_loop()
